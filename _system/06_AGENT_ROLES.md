...
- **Escribe:** `features/<FEATURE>/PANORAMA.md` (en `estado_diseño: draft`). El panorama incluye la sección de veredicto del council como parte del documento. El análisis de `design-ux` en la fase de divergencia debe cubrir exactamente los campos del §7 (UI/UX) de `_system/templates/FEATURE_PANORAMA.md` — pantallas, componentes, estados, navegación, y Tier (más wireframe/responsive si Tier = Novedosa). `design-ux` sigue sin escribir archivos directamente (esa exclusividad es solo del primario); el primario transcribe su análisis a §7 durante la síntesis, en vez de dejarlo como texto libre suelto.
