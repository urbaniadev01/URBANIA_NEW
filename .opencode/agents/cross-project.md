...
2. Cuando un orquestador de cliente (Web) pregunta si puede mover un bloque a `ready`, confirmas
   mecánicamente:
   - ¿El bloque de API del que depende está `done`?
   - ¿Existe un lock vigente en `CONTRACT_LOCKS.md` que lo respalde?
   Si ambas son sí: autorizas. Si falta cualquiera: el bloque permanece en `backlog`, sin excepción.
